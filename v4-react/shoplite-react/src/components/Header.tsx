  ...
  return (
    <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:px-6 lg:px-8">
        <Link to="/" className="shrink-0 text-xl font-black tracking-tight text-blue-700 hover:text-blue-800 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-blue-600">
          ShopLite
        </Link>
        <SearchBar query={query} onQueryChange={onQueryChange} />
        <Link to="/cart" className="relative flex shrink-0 items-center gap-2 rounded-full p-2 text-slate-700 transition hover:bg-blue-50 hover:text-blue-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600" aria-label={`Giỏ hàng, ${cartCount} sản phẩm, tổng ${currencyFormatter.format(cartTotal)}`}>
